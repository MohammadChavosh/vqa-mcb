...
def qlstm(mode, batchsize, max_words_in_question, question_vocab_size):
    ...
    for i in range(max_words_in_question-1):
    ...
    n.lstm1_reshaped_drop = L.Dropout(n.lstm1_reshaped,dropout_param={'dropout_ratio':0.3})
    ...
    n.lstm2_reshaped = L.Reshape(n.lstm2_out,\
                          reshape_param=dict(\
                              shape=dict(dim=[-1,1024])))
    n.lstm2_reshaped_drop = L.Dropout(n.lstm2_reshaped,dropout_param={'dropout_ratio':0.3})
