...
class VQADataProvider:
    ...
    def create_batch(self,qid_list):
        ...
        ivec = (np.zeros(self.batchsize*512*14*14)).reshape(self.batchsize,512,14,14)
        ...
        for i,qid in enumerate(qid_list):
            ...
            try:
                ...
                if data_split == 'genome':
                    raise ValueError('VGG for Visual Genome is not supported')
                else:
                    t_ivec = np.load(config.DATA_PATHS[data_split]['features_prefix'] + str(q_iid).zfill(12) + '.npy')
                    t_ivec = t_ivec.transpose((0, 3, 1, 2))  # NHWC -> NCHW
...
class VQADataProviderLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        top[2].reshape(self.batchsize,512,14,14)
